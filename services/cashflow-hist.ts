...
const BASE_CASHFLOW_HIST_API_URL = `${BASE_URL}/api/${API_VERSION}/cashflow-hist`;

export const getCashflowHistAPI = async (
  sess: Session | null,
  params: IStockGetReq
) => {
  if (!sess) return null;
  ...
  const reqURL = `${BASE_CASHFLOW_HIST_API_URL}?${qStr}`;
